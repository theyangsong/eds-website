...
const overflowTooltipScopeClass = computed(
  () => `showcaseTokens eds-overflow-text-tooltip ${styles.overflowTooltip}`,
);
...
function measureOverflow() {
  ...
  const el =
    props.family === 'custom' ? customContentRef.value : rootRef.value;
  if (!el) {
    overflowing.value = false;
    tooltipText.value = '';
    return;
...
<template>
  ...
  >
    <span ref="rootRef" :class="tagClasses">
      <span v-if="family === 'custom'" ref="customContentRef" :class="styles.customContent">
        <slot />
      </span>
      <slot v-else />
    </span>
  ...
  <span v-else :class="tagClasses">
    <span v-if="family === 'custom'" :class="styles.customContent">
      <slot />
    </span>
    <slot v-else />
  </span>
